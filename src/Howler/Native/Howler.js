...
Elm.Native.Howler.make = function(localRuntime) {
    ...
    function create(soundLabel, audioObj) {
        return Task.asyncFunction(function(callback) {
            ...
            // var listeners = List.toArray(audioObj.eventListeners);
            // for(var listener of listeners) {
            //     var playObj = { soundLabel: soundLabel, playId: Maybe.Nothing }
            //     var listenerFn = function() {
            //         return callback(
            //             Task.succeed(
            //                 listener.wrapperFunction(
            //                     Utils.Tuple2(playObj, listener.event)
            //             )));
            //     }

            //     if(listener.oneOff) newHowl.once(listener.event, listenerFn);
            //     else newHowl.on(listener.event, listenerFn);
            // }
